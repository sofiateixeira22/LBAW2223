...
> Data modeling is the process of creating a data model for the data to be stored in a database. A Conceptual Data Model is an organized view of database concepts and their relationships. The purpose of creating a conceptual data model is to establish entities, their attributes, and relationships.

### 1. Class diagram
The UML Class diagram is a graphical notation used to construct and visualize object oriented systems. It contains classes, their attributes, operations (or methods), and the relationships among objects.

![class_diagram](uploads/a65ec0a719206131363ed9e9e172595d/class_diagram.png)

### 2. Additional Business Rules
 
- BR01: An answer can only be marked as correct by the owner of its parent post
...
| ------------------ | ------------------------- |
| R01 | users(<u>id</u>, name, email **UK NN**, username **UK NN**, password **NN**, birthday **NN**, isDeleted **NN DF false**) |
| R02 | roles(userID->users **UK NN**, userRole **NN CK userRole IN user_role**) |
| R03 | posts(<u>id</u>, userID->users **NN**, postDate **NN DF now()**, postType **NN CK postType IN post_type**, tittle **CK postType IN post_type**, postText **NN**, parentPost->posts, isCorrect **CK postType IN post_type DF false**) |
| R04 | stars(postID->posts **NN**, userID->users **NN**) |
| R05 | comments(<u>id</u>, postID->posts **NN**, userID->users **NN**, commentText **NN**, commentDate **NN DF now()**) |
| R06 | reports(<u>id</u>, reportType **NN CK reportType IN report_type**, userID->users, postID->posts, commentID->comments)
| R07 | tags(<u>id</u>, tagName **UK NN**) |
| R08 | user_tags(userID->users **NN**, tagID->tags **NN**) |
| R09 | question_tags(postID->posts **NN**, tagID->tags **NN**) |
| R10 | badges(<u>id</u>, badgeName **UK NN**) |
| R11 | user_badges(userID->users **NN**, badgeID->badges **NN**) |
| R12 | notifications(<u>id</u>, userID->users **NN**, isRead **NN**, notificationDate **NN DF now()**) |
| R13 | new_answers(notificationID->notifications **NN**, postID->posts **NN**, questionID->posts **NN**) |
| R14 | new_questions(notificationID->notifications **NN**, postID->posts **NN**) |
| R15 | new_comments(notificationID->notifications **NN**, commentID->comments **NN**) |
| R16 | new_badges(notificationID->notifications **NN**, badgeID->badges **NN**) |
| R17 | new_stars(notificationID->notifications **NN**, postID->posts **NN**) |
| R18 | user_questions(userID->users **NN**, postID->posts **NN**) |
...
| user_roles  | ENUM('Moderator', 'Administrator') |
| report_type | ENUM('post', 'comment')            |
...
| FD0101 | {id} -> {name, email, username, password, birthday, isDeleted} |
| FD0102 | {email} -> {id, name, username, password, birthday, isDeleted} |
| FD0103 | {username} -> {id, name, email, password, birthday, isDeleted} |
| **Normal Form** | BCNF |

| **TABLE R02** (roles) |  |
|--|--|
| **Keys** | {userID}  |
| **Functionalities Dependencies** | {userID} -> {user_role} |
| **Normal Form** | BCNF |
...
| **Keys** | {id} |
| **Functionalities Dependencies** |  |
| FD0301 | {id} -> {userID, postDate, postType, title, postText, parentPost, isCorrect} |
| **Normal Form** | BCNF |

| **TABLE R04** (stars) |  |
|--|--|
| **Keys** | {postID, userID} |
| **Functionalities Dependencies** | *none* |
| **Normal Form** | BCNF |
...
| **Keys** | {id} |
| **Functionalities Dependencies** |  |
| FD0501 | {id} -> {postID, userID, commentText, commentDate} |
| **Normal Form** | BCNF |
...
| **Keys** | {id} |
| **Functionalities Dependencies** |  |
| FD0601 | {id} -> {reportType, userID, postID, commentID} |
| **Normal Form** | BCNF |
...
| **Keys** | {id} |
| **Functionalities Dependencies** |  |
| FD1201 | {id} -> {userID, isRead, notificationDate} |
| **Normal Form** | BCNF |


| **TABLE R13** (new_answers) ||
|--|--|
| **Keys** | {notificationID} |
| **Functionalities Dependencies** |  |
| FD1301 | {notificationID} -> {postID, questionID} |
| **Normal Form** | BCNF |

| **TABLE R14** (new_questions) ||
|--|--|
| **Keys** | {notificationID} |
| **Functionalities Dependencies** |  |
| FD1401 | {notificationID} -> {postID, tagID} |
| **Normal Form** | BCNF |

| **TABLE R15** (new_comments) ||
|--|--|
| **Keys** | {notificationID} |
| **Functionalities Dependencies** |  |
| FD1501 | {notificationID} -> {commentID} |
| **Normal Form** | BCNF |

| **TABLE R16** (new_badges) ||
|--|--|
| **Keys** | {notificationID} |
| **Functionalities Dependencies** |  |
| FD1601 | {notificationID} -> {badgeID} |
| **Normal Form** | BCNF |
...
> The identification and characterization of indexes, triggers and transactions are described in this artifact. It contains the physical schema of the database.

### 1. Database Workload
This section describes how the database can be analyzed for memory use. It can give a small approximation of the database performance.

| **Relational Reference** | **Relation Name**  | **Order of magnitude** | **Estimated Growth** |
|--------------------------|--------------------|------------------------|----------------------|
| R01                      | users              | tens of thousands      | dozens/day           |
| R02                      | roles              | units                  | none                 |
| R03                      | posts              | tens of thousands      | dozens/day           |
| R04                      | stars              | hundreds of millions   | hundreds/day         |
| R05                      | comments           | tens of thousands      | hundreds/day         |
| R06                      | reports            | hundreds               | dozens/week          |
| R07                      | tags               | hundreds               | units/week           |
| R08                      | user_tags          | millions               | dozens/day           |
| R09                      | question_tags      | hundreds of thousands  | dozens/day           |
| R10                      | badges             | hundreds               | almost none          |
| R11                      | user_badges        | millions               | dozens/day           |
| R12                      | notifications      | billions               | tens of thousands/day|
| R13                      | new_answers        | millions               | hundreds/day         |
| R14                      | new_questions      | hundreds of millions   | hundreds/day         |
| R15                      | new_comments       | hundreds of millions   | hundreds/day         |
| R16                      | new_badges         | millions               | dozens/day           |
| R17                      | new_stars          | billions               | thousands/day        |
| R18                      | user_questions     | millions               | hundreds/day         |
...
In our case, we find that there is only one feature that requires a performance index:
...
| Cardinality   | high    |
| Clustering    | No      |
| Justification | Table "tags" is frequently accessed to check the tags for a post, or what tags does a user follow. Filtering is done by exact match, thus a hash type is best suited. |
| SQL Code      | ```CREATE INDEX search_tags ON tags USING hash(tagName);``` |


#### 2.2. Full-text Search Indices 
A full-text index stores information about significant words and their location within one or more columns of a database table.
...
| Type          | GIN            |
| Clustering    | No             |
| Justification | Provides full-text search features to look for users based on matching names and usernames. The index type is GIN because the indexed fields are not expected to change often. |
```SQL
ALTER TABLE users
...
| ------------- | ----------------------- |
| Relation      | posts                   |
| Attribute     | title, postText, userID |
| Type          | Gin                     |
| Clustering    | No                      |
| Justification | Provides full-text search features to look for posts based on matching title, text and userID (author of the post). The index type is GIN because the indexed fields are not expected to change often. |
```SQL
ALTER TABLE posts
ADD COLUMN tsvectors TSVECTOR;
...
| Type          | Gin         |
| Clustering    | No          |
| Justification | Provides full-text search features to look for comments based on matching text. The index type is GIN because the indexed fields are not expected to change often. |
```SQL
...
| Trigger     | TGR01 |
| ----------- | ----- |
| Description | An author receives a notification when an answer to their question is posted. |
```SQL
...
| Trigger     | TGR02 |
| ----------- | ----- |
| Description | A user receives a notification when a question is posted and has an associated tag that the user follows. |
```SQL
...
| Trigger     | TGR03 |
| ----------- | ----- |
| Description | An author receives a notification when a comment is inserted on their post. |
```SQL
...
| Trigger     | TGR04 |
| ----------- | ----- |
| Description | An author receives a notification when another user votes on their post. |
```SQL
...
| Trigger     | TGR05 |
| ----------- | ----- |
| Description | A user receives a notification when they receive a badge. |
```SQL
...
| Trigger     | TGR06 |
| ----------- | ----- |
| Description | When a post is edited, it needs to be set as such. |
```SQL
...
| Transaction     | TRAN01 |
| --------------- | ------ |
| Description     | Get questions posted by own user |
| Justification   | The insertion of new rows into the ```posts``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
...
| Transaction     | TRAN02 |
| --------------- | ------ |
| Description     | Get answers posted by own user |
| Justification   | The insertion of new rows into the ```posts``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
| Isolation Level | SERIALIZABLE READ ONLY |
```SQL
...
| Transaction     | TRAN03 |
| --------------- | ------ |
| Description     | Get comments posted by own user |
| Justification   | The insertion of new rows into the ```comments``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
| Isolation Level | SERIALIZABLE READ ONLY |
```SQL
...
| Justification   | The insertion of new rows into the ```user_badges``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
| Isolation Level | SERIALIZABLE READ ONLY |
```SQL
...
| Transaction     | TRAN05 |
| --------------- | ------ |
| Description     | Get user's followed tags |
| Justification   | The insertion of new rows into the ```user_tags``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
| Isolation Level | SERIALIZABLE READ ONLY |
```SQL
...
| Transaction     | TRAN06 |
| --------------- | ------ |
| Description     | Get questions with most votes |
| Justification   | The insertion of new rows into the ```posts``` table during a transaction suggests that the data that was acquired during select is changed, which leads to a Phantom Read. Because it only employs Selects, it is READ ONLY. |
| Isolation Level | SERIALIZABLE READ ONLY |
```SQL
...
| Justification   | When inserting a question, it's important to make sure that all of this code runs smoothly in order to preserve consistency. Because an insert into the table's content by a concurrent transaction would update the sequence post id seq, causing a question to be associated with the incorrect content, the isolation level is Repeatable Read. |
...
| Justification   | When inserting an answer, it's important to make sure that all of this code runs smoothly in order to preserve consistency. Because an insert into the table's content by a concurrent transaction would update the sequence post id seq, causing an answer to be associated with the incorrect content, the isolation level is Repeatable Read. |
| Isolation Level | REPEATABLE READ |
```SQL
...
| Justification   | When inserting a comment, it's important to make sure that all of this code runs smoothly in order to preserve consistency. Because an insert into the table's content by a concurrent transaction would update the sequence comment id seq, causing a comment to be associated with the incorrect content, the isolation level is Repeatable Read. |
| Isolation Level | REPEATABLE READ |
```SQL
...
> This section contains a database creation script that we used to create our database. This script is included in the group's repository and it's in the production server db.fe.up.pt. It also contains a preview of the population script (first 10 lines), which is used to populate the database and test it.
...
## Revision history
- Database has been changed. Populate has been changed as well to include all of it.
- Triggers have been updated, and added new ones.
- Database workload corrected.
- Added user_questions to relational schema

---

GROUP2294, 31/10/2022
